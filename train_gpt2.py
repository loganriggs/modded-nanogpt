...
if __name__ == "__main__":
    ...
    bilinear_small_setting = {
        "n_layer": 12,
        "n_head": 6,
        "n_embd": 768,
        "bilinear": True,
    }
    bilinear_medium_setting = {
        "n_layer": 18,
        "n_head": 9,
        "n_embd": 1152,
        "bilinear": True,
    }
    bilinear_large_setting = {
        "n_layer": 24,
        "n_head": 12,
        "n_embd": 1536,
        "bilinear": True,
    ...
    # Select setting based on command-line argument
    settings_map = {
        'debug': debug_setting,
        'small': small_setting,
        'medium': medium_setting,
        'large': large_setting,
        'bilinear_small': bilinear_small_setting,
        'bilinear_medium': bilinear_medium_setting,
        'bilinear_large': bilinear_large_setting,
    }
    setting = settings_map[cmd_args.setting]
    assert cmd_args.bilinear == setting["bilinear"] 
    model = GPT(GPTConfig(
        vocab_size=num_vocab,
        n_layer=setting["n_layer"],
        n_head=setting["n_head"],
        n_embd=setting["n_embd"],
    ...
    for step in range(args.num_iterations + 1):
        ...
        if master_process and (last_step or (args.save_every > 0 and step % args.save_every == 0)):
            ...
            checkpoint_dir = 'logs/%s' % run_id
            os.makedirs(checkpoint_dir, exist_ok=True)
            torch.save(log, '%s/state_step%06d.pt' % (checkpoint_dir, step))
            # start the clock again
            torch.cuda.synchronize()
            t0 = time.time()
